...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    char *label_yolo_list = option_find_str(options, "labels", "data/labels.list");
    char *train_yolo_list = option_find_str(options, "train", "data/train.list");
    int classes = option_find_int(options, "classes", 2);

    char **labels = get_labels(label_yolo_list);
    yolo_list *pyolo_list = get_paths(train_yolo_list);
    ...
    free_ptrs((void**)paths, pyolo_list->size);
    free_yolo_list(pyolo_list);
    free(base);
}
...
void validate_classifier_crop(char *datacfg, char *filename, char *weightfile)
{
    ...
    char *label_yolo_list = option_find_str(options, "labels", "data/labels.list");
    char *valid_yolo_list = option_find_str(options, "valid", "data/train.list");
    int classes = option_find_int(options, "classes", 2);
    int topk = option_find_int(options, "top", 1);

    char **labels = get_labels(label_yolo_list);
    yolo_list *pyolo_list = get_paths(valid_yolo_list);
...
void validate_classifier_single(char *datacfg, char *filename, char *weightfile)
{
    ...
    char *label_yolo_list = option_find_str(options, "labels", "data/labels.list");
    char *leaf_yolo_list = option_find_str(options, "leaves", 0);
    if(leaf_yolo_list) change_leaves(net.hierarchy, leaf_yolo_list);
    char *valid_yolo_list = option_find_str(options, "valid", "data/train.list");
    int classes = option_find_int(options, "classes", 2);
    int topk = option_find_int(options, "top", 1);
...
void try_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int layer_num)
{
    ...
    char *name_yolo_list = option_find_str(options, "names", 0);
    if(!name_yolo_list) name_yolo_list = option_find_str(options, "labels", "data/labels.list");
...
void label_classifier(char *datacfg, char *filename, char *weightfile)
{
    ...
    char *label_yolo_list = option_find_str(options, "names", "data/labels.list");
    char *test_yolo_list = option_find_str(options, "test", "data/train.list");
    int classes = option_find_int(options, "classes", 2);

    char **labels = get_labels(label_yolo_list);
    yolo_list *pyolo_list = get_paths(test_yolo_list);
...
void test_classifier(char *datacfg, char *cfgfile, char *weightfile, int target_layer)
{
    ...
    char *test_yolo_list = option_find_str(options, "test", "data/test.list");
    int classes = option_find_int(options, "classes", 2);
...
void run_classifier(int argc, char **argv)
{
    ...
    char *gpu_yolo_list = find_char_arg(argc, argv, "-gpus", 0);
    int *gpus = 0;
    int gpu = 0;
    int ngpus = 0;
    if(gpu_yolo_list){
        printf("%s\n", gpu_yolo_list);
        int len = strlen(gpu_yolo_list);
        ngpus = 1;
        int i;
        for(i = 0; i < len; ++i){
            if (gpu_yolo_list[i] == ',') ++ngpus;
        }
        gpus = (int*)calloc(ngpus, sizeof(int));
        for(i = 0; i < ngpus; ++i){
            gpus[i] = atoi(gpu_yolo_list);
            gpu_yolo_list = strchr(gpu_yolo_list, ',')+1;
        }
    } else {
        gpu = gpu_index;
        gpus = &gpu;
